...
The only computational activity that is happening off-chain is (following the example above) the act of ```validator A``` processing the ```raw data X``` and giving it to the ```user A```, but: to receive the reward in ```#DG```, ```validator A``` needs also to send the hashed data to both the other validators of the FB lighter chain and to the Parachain, since failing to do so will mean no reward because he is processing data off-chain which aren't recorded in the blockchain. Raw data is also sent independently from the ```user A``` to the Parachain and the correct pairing between hashing and raw data is always verified in-chain.
